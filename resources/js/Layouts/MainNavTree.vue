      ...
      <template #item="{ item }">
        <div :class="['flex items-center cursor-pointer group', { active: page.url === item.href }]">
          ...
          <div class="ml-auto flex items-center gap-2">
            <Badge
              v-if="item.key === 3"
              :value="dueAgreementsCount > 0 ? dueAgreementsCount : '0'"
              :severity="dueAgreementsCount > 0 ? 'danger' : 'info'"
              class="ml-auto"
            />
            <span v-if="item.shortcut" class="rounded text-muted-color text-xl p-1 w-8">
...
const getPaymentStatus = (schedule) => {
  ...
  const today = moment();
  const dueDate = moment(schedule.due_date, ["YYYY-MM-DD", "DD/MM/YYYY", moment.ISO_8601], true);
  if (!dueDate.isValid()) {
