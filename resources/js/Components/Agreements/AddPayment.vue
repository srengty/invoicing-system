<template>
    <div class="flex flex-col gap-3 py-2">
        ...
        <FloatLabel variant="on">
            <Textarea id="short_description" v-model="model.short_description" fluid ></Textarea>
        ...
        <div class="flex items-center gap-2">
            ...
            <RadioButtonGroup id="currency" v-model="model.currency" class="flex flex-wrap gap-4 w-100" @value-change="doCurrencyChange">
                <div class="flex items-center gap-2" v-for="currency in currencies" :key="currency">
                    <RadioButton :inputId="currency.value" :value="currency.value" />
                    <label :for="currency.value">{{ currency.name }}</label>
                </div>
            </RadioButtonGroup>
        </div>
        <FloatLabel variant="on" v-if="model.currency!=model.agreement_currency">
            <InputNumber v-model="model.exchange_rate" fluid :maxFractionDigits="6"  id="exchange_rate"/>
            <label for="exchange_rate" class="required z-10">Rate</label>
        </FloatLabel>
        <div class="flex justify-end gap-2">
            <Button label="Save" class="grow" @click="doSave"></Button>
            <Button label="Cancel" severity="secondary" @click="emit('cancel')" ></Button>
        </div>
    </div>
</template>
...
onMounted(()=>{
    ...
    if(model.value.currency!=model.value.agreement_currency){
        model.value.exchange_rate = rates[`${model.value.agreement_currency}${model.value.currency}`];
        amountPercentage.value.amount = amountPercentage.value.amount * model.value.exchange_rate;
    }
});
const doAmountChange = (e) => {
    const amount = model.value.agreement_amount*rates[`${model.value.agreement_currency}${model.value.currency}`];
    amountPercentage.value.percentage = Math.round(((e.value?e.value:model.value.amount)/amount)*10000)/100;
}
const doPercentageChange = (e) => {
    const amount = model.value.agreement_amount*rates[`${model.value.agreement_currency}${model.value.currency}`];
    amountPercentage.value.amount = Math.round(((e.value?e.value:model.value.percentage)/100)*amount*100)/100;
}
const doCurrencyChange = (e) => {
    console.log(`${model.value.agreement_currency}${model.value.currency}`);
    if(model.value.currency!=model.value.agreement_currency){
        model.value.exchange_rate = rates[`${model.value.agreement_currency}${model.value.currency}`];
        amountPercentage.value.amount = (model.value.agreement_amount*amountPercentage.value.percentage/100)*model.value.exchange_rate;
    ...
    else amountPercentage.value.amount = (model.value.agreement_amount*amountPercentage.value.percentage/100);
}
const doSave = () => {
