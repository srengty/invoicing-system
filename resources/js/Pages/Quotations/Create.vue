...
import Toast from 'primevue/toast';
  ...
  // Local reactive state for product selection
  const selectedProductIds = ref([]);
  // This array will hold full product objects augmented with qty and subTotal.
  ...
  // When the MultiSelect selection changes, update the selectedProductsData.
  // Add new products with a default quantity and remove ones that are no longer selected.
  ...
  // Update a product’s subtotal when its quantity changes.
  const updateProductSubtotal = (row) => {
    // Ensure the quantity is a number (default to 0 if not)
  ...
  // Update tax and recalc grand total.
  const updateTax = () => {
    form.grand_total = calculateGrandTotal.value;
  };
  
  // Computed total: sum of all subtotals.
  ...
  // Computed grand total (here simply total plus tax).
  const calculateGrandTotal = computed(() => {
    return calculateTotal.value + Number(form.tax || 0);
  });
  
  // Format customers for the Select input.
