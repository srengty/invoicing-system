<template>
  ...
  >
    ...
    <div class="flex flex-row justify-between mb-6">
      ...
      <div class="flex flex-col w-1/2 items-end gap-4">
        <div class="right grid gap-4">
          <p><strong>Quotation No:</strong> {{ invoice.quotation_no || "N/A" }}</p>
          <p><strong>Agreement No:</strong> {{ invoice.agreement_no || "N/A" }}</p>
        </div>
      </div>
    </div>
    ...
    <div ref="printTable" class="page-break">
       <div v-if="Object.keys(groupedPaymentSchedules).length" class="table-container">
  <table class="table-fixed w-full border border-gray-300 text-sm text-center">
    <thead class="bg-gray-100">
      <tr>
        <th class="w-1/4 border border-gray-300 px-4 py-2">Agreement No.</th>
        <th class="w-1/4 border border-gray-300 px-4 py-2">Payment No.</th>
        <th class="w-1/4 border border-gray-300 px-4 py-2">Amount</th>
        <th class="w-1/2 border border-gray-300 px-4 py-2">Description</th>
      </tr>
    </thead>
    <tbody>
      <template v-for="(payments, agreementNo) in groupedPaymentSchedules" :key="agreementNo">
        <tr v-for="payment in payments" :key="payment.id" class="break-inside-avoid">
          <td class="border border-gray-300 px-4 py-3">{{ agreementNo }}</td>
          <td class="border border-gray-300 px-4 py-3">
            {{ formatPaymentNumber(payment.sequenceNumber) }}
          </td>
          <td class="border border-gray-300 px-4 py-3">
    ...
    <div v-else-if="invoice.products && invoice.products.length" class="table-container">
            ...
            <div class="font-medium">
              {{
                isUSD
                  ...
                  : product.product_name_kh || product.product_name
              }}
            </div>
...
const totalAmount = computed(() => {
  const totalKHR =
    ...
    invoice.value.products?.reduce(
      (sum, product) => sum + (product.pivot?.quantity || 0) * (product.pivot?.price || 0),
      0
    ) ||
    0;
...
const formatPaymentNumber = (num) => {
  ...
  const v = num % 100;
  const suffix =
    suffixes[(v % 10)] && ![11, 12, 13].includes(v)
      ? suffixes[v % 10]
      : 'th';
